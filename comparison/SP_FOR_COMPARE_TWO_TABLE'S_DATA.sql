...
BEGIN
	...
	BEGIN TRY
		...
		SELECT ROW_NUMBER() OVER (ORDER BY [COLUMN_NAME]) [ColumnOrder], [COLUMN_NAME] FROM (
		SELECT [COLUMN_NAME] FROM INFORMATION_SCHEMA.COLUMNS WHERE [TABLE_SCHEMA] = PARSENAME(@RightTable,2) AND [TABLE_NAME] = PARSENAME(@RightTable,1)
		INTERSECT
		SELECT [COLUMN_NAME] FROM INFORMATION_SCHEMA.COLUMNS WHERE [TABLE_SCHEMA] = PARSENAME(@LeftTable,2) AND [TABLE_NAME] = PARSENAME(@LeftTable,1)
		) AS SQ1 WHERE QUOTENAME(SQ1.COLUMN_NAME) NOT IN (SELECT [value] FROM STRING_SPLIT(@IgnoreColumns, ',')) --Order based on ordinal position need to work...
